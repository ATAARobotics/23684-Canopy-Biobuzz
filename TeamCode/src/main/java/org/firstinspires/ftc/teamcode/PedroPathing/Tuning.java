package org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing;

import static com.aaravlabs.safepedropathing.math.MathFunctions.quadraticFit;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.changes;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.drawCurrent;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.drawCurrentAndHistory;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.follower;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.stopRobot;
import static org.firstinspires.ftc.teamcode.aaravlabs.safepedropathing.Tuning.telemetryM;
...
import com.aaravlabs.safepedropathing.geometry.*;
import com.aaravlabs.safepedropathing.math.*;
import com.aaravlabs.safepedropathing.paths.*;
import com.aaravlabs.safepedropathing.telemetry.SelectableOpMode;
import com.aaravlabs.safepedropathing.util.*;
...
class LocalizationTest extends OpMode {
	...
	public void init_loop() {
		...
		telemetryM.debug("Drivetrain debug string " + (((debugStringEnabled) ? "enabled" : "disabled")) + " (press gamepad a to toggle)");
...
class ForwardTuner extends OpMode {
	...
	public void loop() {
		...
		telemetryM.debug("The multiplier will display what your forward ticks to inches should be to scale your current distance to "
				+ DISTANCE
				+ " inches.");
		telemetryM.debug("Multiplier: "
				+ (DISTANCE / (follower.getPose().getX() / follower.getPoseTracker().getLocalizer().getForwardMultiplier())));
		telemetryM.update(telemetry);
...
class LateralTuner extends OpMode {
	...
	@Override
	public void init_loop() {
		telemetryM.debug("Pull your robot to the right "
				+ DISTANCE
	...
	public void loop() {
		...
		telemetryM.debug("The multiplier will display what your strafe ticks to inches should be to scale your current distance to "
				+ DISTANCE
				+ " inches.");
		telemetryM.debug("Multiplier: "
				+ (DISTANCE / (follower.getPose().getY() / follower.getPoseTracker().getLocalizer().getLateralMultiplier())));
		telemetryM.update(telemetry);
...
class TurnTuner extends OpMode {
	...
	public void loop() {
		...
		telemetryM.debug("The multiplier will display what your turn ticks to inches should be to scale your current angle to "
				+ ANGLE
				+ " radians.");
		telemetryM.debug("Multiplier: "
				+ (ANGLE / (follower.getTotalHeading() / follower.getPoseTracker().getLocalizer().getTurningMultiplier())));
		telemetryM.update(telemetry);
	...
	public void loop() {
		...
		switch (state) {
			...
			case RECORD: {
				...
				telemetryM.debug("Test " + iteration, String.format("v=%.3f  d=%.3f", measuredVelocity, brakingDistance));
				telemetryM.update(telemetry);
...
class TranslationalTuner extends OpMode {
	...
	public void loop() {
		...
		telemetryM.addData(
				"Error X", follower.errorCalculator.getTranslationalError().getXComponent());
		telemetryM.addData(
				"Error Y", follower.errorCalculator.getTranslationalError().getYComponent());
		telemetryM.update(telemetry);
	}
}
...
class CentripetalTuner extends OpMode {
	...
	@Override
	public void init_loop() {
		telemetryM.debug("This will run the robot in a curve going "
				+ DISTANCE
